...
namespace ini
{
    ...
    {
        void decode(const char * &value, std::string &result) {
            result = value;
        }

        void encode(const std::string value, const char * &result) {
            result = value.c_str();
        }
    };
    ...
    public:
        ...
        IniFileBase(const char fieldSep, const char comment) : fieldSep_(fieldSep), commentPrefixes_(1, std::string(1, comment)) 
        {}

        IniFileBase(const std::string &filename, const char fieldSep = '=', const char comment = '#') : IniFileBase(fieldSep, comment) {
            load(filename);
        }

        IniFileBase(std::istream &is, const char fieldSep = '=', const char comment = '#') : IniFileBase(fieldSep, comment) {
            decode(is);
        }

        IniFileBase(const char fieldSep, const std::vector<std::string> &commentPrefixes) : fieldSep_(fieldSep), commentPrefixes_(commentPrefixes) {
        }

        IniFileBase(const std::string &filename, const char fieldSep, const std::vector<std::string> &commentPrefixes) : fieldSep_(fieldSep), commentPrefixes_(commentPrefixes) {
            load(filename);
        }
        ...
        void setfieldSep(const char seq) {
            fieldSep_ = seq;
        }
